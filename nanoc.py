...
commande: commande (";" commande)*   -> sequence
    ...
    | "skip"                                  -> skip
fonction: IDENTIFIER "(" liste_var ")" "{" commande "return" "(" expression ")" "}" -> fonction
program: (fonction)* "main" "(" liste_var ")" "{"commande"return" "("expression")" "}"
...
def pp_fonction(f) :
    vars = f.children[0]
    return f"{f.children[1]}({pp_liste_vars(vars)}) {{\n{pp_commande(f.children[2])}\nreturn {pp_expression(f.children[3])}\n}}  "
